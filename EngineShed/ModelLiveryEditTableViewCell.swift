//
//  ModelLiveryEditTableViewCell.swift
//  EngineShed
//
...
class ModelLiveryEditTableViewCell : UITableViewCell, UITextFieldDelegate {
    ...
    func textField(_ textField: UITextField, shouldChangeCharactersIn range: NSRange, replacementString string: String) -> Bool {
        ...
        if !string.isEmpty,
            let suggestion = model?.suggestionsForLivery(startingWith: text).first
        {
    ...
    func observeModel() {
        ...
        observers.append(model.observe(\.livery) { (_, _) in
            DispatchQueue.main.async {
                self.textField.text = model.livery
            }
        })
    }
