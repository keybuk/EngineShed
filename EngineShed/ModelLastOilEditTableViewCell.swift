//
//  ModelLastOilEditTableViewCell.swift
//  EngineShed iOS
//
...
class ModelLastOilEditTableViewCell : UITableViewCell, UITextFieldDelegate {
    ...
    func configureView() {
        textField.text = model?.lastOilAsString
        clearButton.isHidden = model?.lastOil == nil
    }
    ...
    func textFieldDidEndEditing(_ textField: UITextField) {
        ...
        // FIXME: date
        model?.lastOilAsString = textField.text
    ...
    func observeModel() {
        ...
        observers.append(model.observe(\.lastOil) { (_, _) in
            DispatchQueue.main.async {
                self.textField.text = model.lastOilAsString
                self.clearButton.isHidden = model.lastOil == nil
            }
        })
    }
