//
//  ModelSocketEditTableViewCell.swift
//  EngineShed
//
...
class ModelSocketEditTableViewCell : UITableViewCell, UITextFieldDelegate {
    ...
    func textField(_ textField: UITextField, shouldChangeCharactersIn range: NSRange, replacementString string: String) -> Bool {
        ...
        if !string.isEmpty,
            let suggestion = model?.suggestionsForSocket(startingWith: text).first
        {
    ...
    func observeModel() {
        ...
        observers.append(model.observe(\.socket) { (_, _) in
            DispatchQueue.main.async {
                self.textField.text = model.socket
            }
        })
    }
