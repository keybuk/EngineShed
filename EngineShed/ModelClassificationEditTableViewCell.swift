//
//  ModelClassificationEditTableViewCell.swift
//  EngineShed iOS
//
...
class ModelClassificationEditTableViewCell : UITableViewCell, UITextFieldDelegate {
    ...
    func configureView() {
        textField.text = model?.classification.flatMap { "\($0)" }
    }
    ...
    func textFieldDidEndEditing(_ textField: UITextField) {
        ...
        // FIXME: enum
        //        model?.classification = textField.text
    }
    ...
    func observeModel() {
        ...
        observers.append(model.observe(\.classificationRawValue) { (_, _) in
            DispatchQueue.main.async {
                self.textField.text = model.classification.flatMap { "\($0)" }
            }
        })
    }
