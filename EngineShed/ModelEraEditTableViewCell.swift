//
//  ModelEraEditTableViewCell.swift
//  EngineShed
//
...
class ModelEraEditTableViewCell : UITableViewCell, UITextFieldDelegate {
    ...
    func configureView() {
        textField.text = model?.era.flatMap { "\($0)" }
    }
    ...
    func observeModel() {
        ...
        observers.append(model.observe(\.eraRawValue) { (_, _) in
            DispatchQueue.main.async {
                self.textField.text = model.era.flatMap { "\($0)" }
            }
        })
    }
