//
//  ModelSpeakerEditTableViewCell.swift
//  EngineShed iOS
//
...
class ModelSpeakerEditTableViewCell : UITableViewCell, UITextFieldDelegate {
    ...
    func textField(_ textField: UITextField, shouldChangeCharactersIn range: NSRange, replacementString string: String) -> Bool {
        ...
        if !string.isEmpty,
            let suggestion = model?.suggestionsForSpeaker(startingWith: text).first
        {
    ...
    func observeModel() {
        ...
        observers.append(model.observe(\.speaker) { (_, _) in
            DispatchQueue.main.async {
                self.textField.text = model.speaker
            }
        })
    }
